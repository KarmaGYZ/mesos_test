...
$FLINK_DIR/bin/flink run -m localhost:${REST_PORT} -p 1 $TEST_PROGRAM_JAR --resolve-order parent-first --output $TEST_DATA_DIR/out/cl_out_pf
...
$FLINK_DIR/bin/flink run -m localhost:${REST_PORT} -p 1 $TEST_PROGRAM_JAR --resolve-order parent-first --output $TEST_DATA_DIR/out/cl_out_cf_pf
...
$FLINK_DIR/bin/flink run -m localhost:${REST_PORT} -p 1 $TEST_PROGRAM_JAR --resolve-order child-first --output $TEST_DATA_DIR/out/cl_out_cf
