...
DATASTREAM_JOB=$($FLINK_DIR/bin/flink run -m localhost:${REST_PORT} -d -p $ORIGINAL_DOP $TEST_PROGRAM_JAR \
...
# wait_oper_metric_num_in_records SemanticsCheckMapper.0 200 TODO download tm log
sleep 10s
...
DATASTREAM_JOB=$($FLINK_DIR/bin/flink run -m localhost:${REST_PORT} -s $SAVEPOINT_PATH -p $NEW_DOP -d $TEST_PROGRAM_JAR \
  --test.semantics exactly-once \
...
# wait_oper_metric_num_in_records SemanticsCheckMapper.0 200 TODO
sleep 10s
