...
function job_id() {
    ...
    CMD="${FLINK_DIR}/bin/flink run -m localhost:${REST_PORT} -d ${SP} -p ${PARALLELISM} ${TEST_PROGRAM_JAR} \
...
# wait_oper_metric_num_in_records TtlVerifyUpdateFunction.0 ${UPDATE_NUM} 'State TTL test job' TODO
sleep 15s
