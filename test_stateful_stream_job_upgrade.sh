...
function job() {
    DOP=$1
    CMD="${FLINK_DIR}/bin/flink run -m localhost:${REST_PORT} -d -p ${DOP} ${TEST_PROGRAM_JAR} \
...
# wait_oper_metric_num_in_records stateMap2.1 200 TODO
sleep 10s
...
# wait_oper_metric_num_in_records stateMap3.2 200 TODO
sleep 10s
