...
function run_test {
    ...
    QUERYABLE_STATE_PRODUCER_JAR=${END_TO_END_DIR}/flink-queryable-state-test/target/QsStateProducer.jar
    QUERYABLE_STATE_CONSUMER_JAR=${END_TO_END_DIR}/flink-queryable-state-test/target/QsStateClient.jar

    # start app with queryable state and wait for it to be available
    JOB_ID=$(${FLINK_DIR}/bin/flink run -m localhost:${REST_PORT} \
        -p 1 \
